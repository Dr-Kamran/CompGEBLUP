...
#'
#' @slot genotypes Matrix of genotype data (individuals x markers). Dosages centered
#'   on 0 in [-1, 1]: inbreds are -1/0/1; hybrids/imputed data may be continuous.
...
setClass("GBLUPData",
         ...
         validity = function(object) {
           ...
           # Genotype coding check.
           # Inbred lines are coded -1/0/1, but HYBRID genotypes are legitimately
           # CONTINUOUS: an F1 dosage is the average of its two parents, so it can
           # take any value in [-1, 1] (e.g. 0.5 for a locus where one parent is
           # heterozygous or partially fixed, or fractional values from imputation).
           # This is standard genomic-prediction input -- VanRaden's A = ZZ'/2Spq
           # and rrBLUP::A.mat both accept continuous dosages. We therefore check
           # the RANGE, not exact membership, so hybrids and imputed data are valid
           # while genuinely mis-scaled matrices (e.g. 0/1/2) are still caught.
           gvals <- object@genotypes[is.finite(object@genotypes)]
           if (length(gvals)) {
             grange <- range(gvals)
             if (grange[1] < -1.0001 || grange[2] > 1.0001) {
               errors <- c(errors, paste0(
                 "genotypes must lie in [-1, 1] (dosages centered on 0); found range [",
                 round(grange[1], 4), ", ", round(grange[2], 4),
                 "]. If your matrix is on the {0,1,2} scale use M-1; ",
                 "on the {0,1} scale use 2*M-1."
               ))
             }
           }
