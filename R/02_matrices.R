...
#' Uses C++ implementation for speed when available.
#'
#' @param K Square matrix
#' @param tol Tolerance for smallest eigenvalue
#' @param ridge Ridge parameter to add to diagonal (deprecated, use tol instead)
...
make_positive_definite <- function(K, tol = 1e-6, ridge = NULL, use.cpp = TRUE) {
  ...
  # Handle deprecated ridge parameter
  if (!is.null(ridge)) {
    message("'ridge' parameter is deprecated. Using 'tol' instead.")
    tol <- max(tol, ridge)
  }
