...
fit_gblup <- function(gdata, K_matrices, effects = "A", 
  ...
  custom_effects <- setdiff(setdiff(effects, builtin_effects), cov_ae_effects)
  ...
  if (length(cov_ae_effects) > 0) {
    if (is.null(env_kernels)) {
      stop("Covariate-specific G×E effects detected (",
           ...
           ")\n  Use build_env_kernel() to construct Omega matrices.")
    }
    missing_omega <- setdiff(cov_ae_effects, names(env_kernels))
    if (length(missing_omega) > 0) {
      stop("Missing environmental kernels for: ",
           paste(missing_omega, collapse = ", "),
           "\n  Available: ", paste(names(env_kernels), collapse = ", "))
    }
    if (verbose) {
      cat("Covariate-specific G×E effects:\n")
      for (ae in cov_ae_effects) {
        cat(sprintf("  %s: %d × %d environmental similarity matrix\n",
                    ae, nrow(env_kernels[[ae]]), ncol(env_kernels[[ae]])))
      }
    }
  }
...
heritability <- function(model, n_env = NULL) {
  ...
  is_interaction <- (grepl("E$", components) | grepl("^AE_", components)) & !is_env
