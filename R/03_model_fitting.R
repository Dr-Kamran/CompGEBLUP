...
#' 
#' # With fixed effects
#' model_fixed <- fit_gblup(gdata, K, effects = "A", fixed = ~ TESTER)
#' }
fit_gblup <- function(gdata, K_matrices, effects = "A", 
  ...
  # Validate effects
  valid_effects <- c("A", "D", "ENV", "AE", "DE", "AA", "AAE")
  invalid_effects <- setdiff(effects, valid_effects)
  if (length(invalid_effects) > 0) {
    stop("Invalid effects: ", paste(invalid_effects, collapse = ", "),
  ...
  if (!K_already_regularized) {
    design$K_list <- lapply(design$K_list, function(K) {
    ...
    if (verbose) {
      cat("Using pre-regularized K matrices (no additional ridge added)\n")
    }
  }
...
extract_gebvs_from_fit <- function(fit, pheno, design, effects, K_matrices = NULL, all_gids = NULL) {
  ...
  # Find the main genetic effect (A or first in list)
  genetic_idx <- which(design$effect_names %in% c("A", "D", "AA"))[1]
  ...
  if (!is.null(K_matrices) && !is.null(all_gids) && "A" %in% names(K_matrices)) {
    K_A <- K_matrices$A
...
heritability <- function(model, n_env = NULL) {
  ...
  total_var <- var_main + var_interaction_contrib + var_env + var_e
  ...
  # H2 = Vg_main / Vp (interaction reduces selection accuracy)
  H2_mean <- var_main / total_var
  
  # === HERITABILITY FOR VARIANCE PARTITIONING (G×E as genetic) ===
  # H2_total = (Vg_main + Vg_int/n_env) / Vp
  H2_total <- (var_main + var_interaction_contrib) / total_var
  ...
  if (length(a_idx) > 0) {
    ...
    # For selection on mean: only main additive effect
    h2_mean <- var_a / total_var
    
    # For variance partitioning: include A×E
    h2_total <- (var_a + var_ae_contrib) / total_var
  } else {
    h2_mean <- NA
    h2_total <- NA
  }
