...
cv_gblup <- function(gdata, K_matrices = NULL, effects = "A",
                     ...
                     n_folds = 5, n_reps = 1,
                     ridge = 0.001, recompute_K = TRUE, verbose = TRUE,
                     fixed = NULL) {
...
cv_fold <- function(gdata, K_matrices, effects, fold, ridge, 
  ...
  if (recompute_K) {
    ...
    if (any(c("A", "AE", "AA", "AAE") %in% effects)) {
      ...
      K_train <- make_positive_definite(K_train, tol = ridge)
    ...
    if (any(c("D", "DE") %in% effects)) {
      ...
      D_train <- make_positive_definite(D_train, tol = ridge)
    ...
    if (any(c("AA", "AAE") %in% effects)) {
      ...
      AA_train <- K_train * K_train  # Hadamard product
      AA_train <- make_positive_definite(AA_train, tol = ridge)
