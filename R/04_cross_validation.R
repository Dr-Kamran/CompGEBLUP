...
#' @param env_kernels Optional named list of environmental similarity matrices (Omega)
#'   for covariate-specific G×E effects (effects whose names start with \code{"AE_"},
#'   e.g. \code{"AE_temperature"}). Each Omega is an e × e matrix (built with
#'   \code{build_env_kernel()}) whose name matches the corresponding \code{AE_*} effect.
#'   Passed through unchanged to \code{fit_gblup()} in each fold. Because the environmental
#'   kernels are defined over environments (not genotypes), they are the same for every fold
#'   and are \strong{not} recomputed when \code{recompute_K = TRUE}. See \code{fit_gblup()}
#'   and \code{build_env_kernel()} for details.
#'
...
cv_gblup <- function(gdata, K_matrices = NULL, effects = "A",
  ...
  # Validate covariate-specific G×E effects up front (v1.1.0).
  # cv_fold() wraps each fold in tryCatch and swallows errors, so validating
  # here gives the user a single clear message instead of "All folds failed".
  cov_ae_effects <- grep("^AE_", effects, value = TRUE)
  if (length(cov_ae_effects) > 0) {
    if (is.null(env_kernels)) {
      stop("Covariate-specific G×E effects detected (",
           paste(cov_ae_effects, collapse = ", "),
           ") but env_kernels is NULL.\n",
           "  Provide env_kernels = list(",
           paste0(cov_ae_effects, " = Omega", collapse = ", "), ")\n",
           "  where each Omega is an e × e environmental similarity matrix ",
           "(see build_env_kernel()).")
    }
    missing_omega <- setdiff(cov_ae_effects, names(env_kernels))
    if (length(missing_omega) > 0) {
      stop("Missing environmental kernel(s) in env_kernels for: ",
           paste(missing_omega, collapse = ", "),
           "\n  Available in env_kernels: ",
           paste(names(env_kernels), collapse = ", "))
    }
  }
...
#' @param env_kernels Optional named list of environmental similarity matrices (Omega)
#'   for covariate-specific G×E effects (\code{AE_*}). Passed through to \code{fit_gblup()}.
#'   These are environment-level kernels and are identical across folds, so they are used
#'   as-is even when \code{recompute_K = TRUE} (only the genomic kernels are refit per fold).
#'
...
cv_fold <- function(gdata, K_matrices, effects, fold, ridge,
  ...
  if (recompute_K) {
    ...
    # Covariate-specific G×E effects (AE_*) also require the additive kernel,
    # since their design kernel is Kronecker(Omega, A) built in build_design_matrices().
    has_cov_ae <- any(grepl("^AE_", effects))
    if (any(c("A", "AE", "AA", "AAE") %in% effects) || has_cov_ae) {
  ...
  # Add G×E breeding values if available.
  #
  # G×E effects have BLUPs indexed by GID.ENV and must be added to the main
  # genetic GEBV for proper prediction. This covers the built-in interaction
  # effects (AE, DE, AAE) AND covariate-specific G×E effects (AE_*, e.g.
  # AE_temperature) that use user-supplied environmental kernels via env_kernels.
  #
  # v1.1.0: Instead of hard-coding the effect order, we use the model's actual
  # component order (model@varcomp$Component, minus "Residual"), which matches the
  # order of fit$u exactly (both derive from design$effect_names). This makes the
  # block robust to AE_* covariate effects and GID.ENV-level custom kernels, which
  # are appended after the built-in effects in build_design_matrices().
  #
  # Effect classification:
  #   - GID-level genetic main effects (A, D, AA) and any GID-level custom kernel
  #     are already captured by the aggregated GEBV in model@gebv -> skipped here.
  #   - The environment main effect (ENV) is not a genotype-specific term -> skipped.
  #   - Everything else is treated as a candidate G×E term and matched by GID.ENV
  #     name; effects whose levels are not GID.ENV-indexed simply produce no matches
  #     and contribute nothing.
  fit <- model@model
  effect_order <- model@varcomp$Component
  effect_order <- effect_order[effect_order != "Residual"]

  skip_effects <- c("A", "D", "AA", "ENV")
  test_gid_env <- paste(test_with_pred$GID, test_with_pred$ENV, sep = ".")

  for (eff_idx in seq_along(effect_order)) {
    eff_name <- effect_order[eff_idx]
    if (eff_name %in% skip_effects) next

    u_gxe <- fit$u[[eff_idx]]
    if (is.null(u_gxe)) next

    # BLUPs are named by colnames(Z) = the effect's levels (GID.ENV for G×E terms)
    gxe_names <- names(u_gxe)
    if (is.null(gxe_names)) gxe_names <- rownames(u_gxe)
    if (is.null(gxe_names)) next

    gxe_match <- match(test_gid_env, gxe_names)
    valid_match <- !is.na(gxe_match)
    if (!any(valid_match)) next  # not a GID.ENV-indexed effect (or nothing overlaps)
