...
fit_mme <- function(y, X, Z_list, K_list,
  ...
  # For small n: EMMA or observation-space AI-REML
...
fit_mme_emma <- function(y, X, Z, K, tol, ridge, verbose) {
  ...
  return(list(
    ...
    AIC = -2 * emma_result$loglik + 2 * 2,
    BIC = -2 * emma_result$loglik + log(n) * 2,
...
fit_mme_henderson <- function(y, X, Z_list, K_list,
  ...
  if (n_random == 1) {
    cpp_result <- tryCatch({
      ...
      list(
        ...
        AIC = -2 * result_cpp$llik + 2 * 2,
        BIC = -2 * result_cpp$llik + log(n) * 2,
  ...
  for (iter in seq_len(nIters)) {
    ...
    # Full EM: σ²_e = RSS / (n - p), σ²_g = (u'K⁻¹u + tr(C_uu⁻¹ K⁻¹)) / q
    rss <- sum(residuals^2)
    theta_new <- theta
    sigma2_e <- rss / (n - p)
    ...
    # L = -0.5 * [log|C| + log|R| + (y-Xb-Zu)'R⁻¹(y-Xb-Zu)]
    # Simplified: -0.5 * [n*log(σ²_e) + RSS/σ²_e]
    llik <- -0.5 * (n * log(theta_new[n_random + 1]) + rss / theta_new[n_random + 1])
    ...
    # Secondary: LL plateau (confounded components trading variance)
    if (iter > 15 && llik_diff < 0.01) {
      converged <- TRUE
      if (verbose) {
...
fit_mme_henderson_ai <- function(y, X, Z_list, K_list,
  ...
  for (iter in seq_len(nIters_em)) {
    ...
    # σ²_e   = rss / (n - p)                            (simplified EM)
    #
    # The σ²_e update omits tr(C⁻¹·C_data) for numerical stability.
    # Both converge to the same REML fixed point; the simplified form
    # avoids bias from trace approximation in Henderson space.
    # =================================================================
    ...
    # Update σ²_e (simplified EM — stable, same fixed point)
    theta_new[n_random + 1] <- max(rss / (n - p), .MME_CONSTANTS$MIN_VARIANCE)
    ...
    if (iter > 1) {
      ...
      # If LL changed < 0.01 over last 10 iterations, declare converged
      if (iter > 15 && llik_diff < 0.01) {
        converged <- TRUE
        if (verbose) cat("Converged at iteration", iter, 
...
fit_mme_ai_reml <- function(y, X, Z_list, K_list,
  ...
  # Heuristic: first effect (usually additive) gets ~30% variance
  # Additional effects get progressively smaller shares
  # Residual gets ~50%
