...
build_env_qtl_matrices <- function(M, qtl_env_markers, env_names, Ka = NULL,
                                   Omega = NULL) {
  # Omega: optional n_env x n_env environmental covariance for the MAJOR-gene x
  #   environment interaction. NULL (default) reproduces the classic behaviour
  #   Omega = I (environments independent, as in mmGEBLUP). Supplying a structured
  #   Omega -- e.g. a location-recurrent, climate-orthogonalized kernel (CW-RN) --
  #   lets a detected major G x E QTL BORROW across environmentally similar sites
  #   and, crucially, transfer to a NEW location. This is the structured
  #   major-gene G x E extension (sm-GEBLUP).
  n_ind <- nrow(M)
  ...
  # Environment covariance for the major-gene x environment interaction.
  # Default = identity (classic mmGEBLUP). If Omega is supplied, use it: this is
  # the single change that turns unstructured major-gene G x E into STRUCTURED
  # major-gene G x E.
  if (is.null(Omega)) {
    E <- diag(n_env)
  } else {
    if (!all(dim(Omega) == c(n_env, n_env)))
      stop("Omega must be ", n_env, " x ", n_env, " (one row/col per environment).")
    # align to env_names if named
    if (!is.null(rownames(Omega)) && all(env_names %in% rownames(Omega)))
      Omega <- Omega[env_names, env_names, drop = FALSE]
    # ensure symmetric PD (a covariance); ridge the diagonal if needed
    Omega <- (Omega + t(Omega)) / 2
    ev <- min(eigen(Omega, symmetric = TRUE, only.values = TRUE)$values)
    if (ev <= 1e-8) Omega <- Omega + diag(1e-6 - ev, n_env)
    E <- Omega
  }
...
extract_gebvs_qtl <- function(fit, pheno, design, X, Ka, has_qtl, qtl_effects, gdata = NULL) {
  ...
  # Get polygenic BLUPs (named by TRAINED genotypes only)
  u_polygenic <- fit$u[[a_idx]]
  gid_train   <- colnames(design$Z_list[[a_idx]])
  if (is.matrix(u_polygenic)) u_polygenic <- as.vector(u_polygenic)
  names(u_polygenic) <- gid_train
  ...
  # --- Polygenic component WITH projection to untrained genotypes -----------
  # BUG FIX: the previous version did gebv_df$A_polygenic <- u_polygenic[GID],
  # which returns NA for any genotype absent from training (e.g. every masked
  # test genotype in cv_gblup_qtl). GEBV then collapsed to the QTL term alone.
  # We now project polygenic BLUPs onto new genotypes exactly as the main
  # fit_gblup path does:  u_new = K[new, tr] %*% solve(K[tr, tr]) %*% u_tr.
  poly <- rep(NA_real_, nrow(gebv_df))
  in_train <- gebv_df$GID %in% gid_train
  poly[in_train] <- u_polygenic[gebv_df$GID[in_train]]

  new_gids <- setdiff(unique(gebv_df$GID[!in_train]), gid_train)
  if (length(new_gids) > 0) {
    Kfull <- tryCatch(calc_relationship_matrices(gdata, matrices = "A")$A,
                      error = function(e) NULL)
    if (is.null(Kfull) && !is.null(Ka)) Kfull <- Ka
    if (!is.null(Kfull)) {
      tr <- intersect(gid_train, rownames(Kfull))
      nn <- intersect(new_gids, rownames(Kfull))
      if (length(tr) >= 5 && length(nn) > 0) {
        Ktt   <- Kfull[tr, tr]
        Knt   <- Kfull[nn, tr, drop = FALSE]
        u_new <- as.numeric(Knt %*% solve(Ktt + diag(1e-6 * mean(diag(Ktt)),
                                                      length(tr)), u_polygenic[tr]))
        names(u_new) <- nn
        idx <- gebv_df$GID %in% nn
        poly[idx] <- u_new[gebv_df$GID[idx]]
      }
    }
  }
  poly[is.na(poly)] <- 0   # no relationship info -> fall back to mean (0 on centered scale)
  gebv_df$A_polygenic <- poly

  # --- QTL fixed-effect component (marker-based; already valid for new gids) --
  if (has_qtl && !is.null(qtl_effects)) {
    ...
    if (!is.null(M)) {
      ...
      gebv_df$A_qtl[is.na(gebv_df$A_qtl)] <- 0
    } else {
      gebv_df$A_qtl <- 0
    }
  } else {
