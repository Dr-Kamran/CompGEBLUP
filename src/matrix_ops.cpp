...
Rcpp::List fit_henderson_mme_cpp(const arma::vec& y, 
  ...
  for (iter = 1; iter <= max_iter; iter++) {
    ...
    // Trace correction: tr(C⁻¹_uu K⁻¹) — NO σ²_e multiplier in Convention 2
    // NOTE: Previously used sigma2_e * trace(...) which is correct for Convention 1
    // where C⁻¹_uu = σ²_e · PEV. In Convention 2, C⁻¹_uu IS the PEV directly.
    ...
    // Compute REML log-likelihood (approximate)
    double llik = -0.5 * (n * std::log(sigma2_e_new) + rss / sigma2_e_new);
