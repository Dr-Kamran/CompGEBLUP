...
test_that("Henderson's MME is automatically selected for large n when use.emma = TRUE", {
  set.seed(12345)
  
  # Create data with large n, small q to trigger Henderson
  ...
  # Fit model with use.emma = TRUE (default) - should automatically use Henderson
  expect_output(
    model <- fit_gblup(gdata, K, effects = "A", verbose = TRUE, use.emma = TRUE),
    "Henderson's MME"
  )
