...
**CompGEBLUP** (Comprehensive Genomic Prediction with GBLUP) is an R package for genomic selection and prediction in plant and animal breeding. It implements five core genomic prediction models with varying complexity, from simple additive models to complex models including dominance, epistasis, and genotype-by-environment (G×E) interactions.
...
- **Cross-validation** - CV0, CV1, CV2 schemes with proper data handling
...
Includes both additive and dominance genetic effects.
...
## Model 4: Full G×E (A + D + ENV + AE + DE)

Comprehensive model with additive and dominance effects plus their environmental interactions.
...
## Model 5: Epistasis (A + D + ENV + AE + AA)

Most complex model including additive-by-additive epistatic interactions.

```{r model5, warning=FALSE}
# Epistasis model - may require large datasets for stable estimation
model5 <- tryCatch({
  fit_model5_ADAA(gdata, include_AAE = FALSE, verbose = FALSE)
}, error = function(e) {
